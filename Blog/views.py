...
def cybersecurity_case_studies(request):
    name="case_studies"
    posts = models.Post.objects.filter(
        Q(category__category__iexact=str(name).replace('_', ' ')) | Q(category__category__iexact=str(name)))
    cat_path = str(request.path).split('/')[-2]
...
def get_context(name1,name2,name3):
...
def category_business_detailview(request,name3):

    name1='case_studies'
    name2='business_cybersecurity'

    context=get_context(name1, name2, name3)

    return render(request, 'blog/category.html', context)

def category_personal_detailview(request,name3):

    name1='case_studies'
    name2='personal_cybersecurity'
